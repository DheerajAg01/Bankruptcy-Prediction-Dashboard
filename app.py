...
# ----------------------------------------------------
# STREAMLIT PAGE CONFIG — must be the FIRST UI call
# ----------------------------------------------------
st.set_page_config(
...
# ----------------------------------------------------
# SESSION INITIALIZATION (before any usage)
# ----------------------------------------------------
...
def fetch_price_history(ticker: str, years: int = 5, retries: int = 3, pause: float = 0.7) -> pd.DataFrame:
    """
    Robust price fetch with multiple fallback strategies
    """
    if yf is None:
        return pd.DataFrame()
...
FEATURES = [
    ...
    "market_cap_to_liabilities", "sales_to_assets",
    "current_ratio", "debt_to_equity", "return_on_assets", "profit_margin",
...
def chart_price(df: pd.DataFrame, ticker: str):
    """Price chart with robust handling"""
    if df is None or df.empty:
        return None
    ...
    try:
        ...
        if "Volume" in df.columns:
            ...
            if not valid_vol.empty:
                if have_ohlc:
                    colors = []
                    for _, row in valid_vol.iterrows():
...
def chart_z_components(components: dict):
    ...
    fig.update_layout(
        title="Altman Z-Score Components", template="plotly_dark", height=400,
...
def chart_ratio_radar(r: dict):
    keys = ["current_ratio", "quick_ratio", "cash_ratio", "debt_to_equity", "interest_coverage",
            "profit_margin", "operating_margin", "asset_turnover"]
    labels = ["Current Ratio", "Quick Ratio", "Cash Ratio", "Debt/Equity", "Interest Coverage",
              "Profit Margin", "Operating Margin", "Asset Turnover"]
    ...
    fig.add_trace(go.Scatterpolar(
        r=clipped, theta=labels, fill="toself", name="Ratios",
        line=dict(color="#6366f1", width=2), fillcolor="rgba(99,102,241,0.3)"
    ))
    fig.update_layout(
        template="plotly_dark", title="Key Financial Ratios Overview", height=450,
        polar=dict(
...
def chart_model_probs(ml: dict):
    rows = [{"Model": k.replace("_", " ").title(),
             ...
             "Safe": v["probability_safe"]} for k, v in ml["individual_models"].items()]
    df = pd.DataFrame(rows)
    fig = go.Figure()
    fig.add_trace(go.Bar(
        name="Bankruptcy Risk", x=df["Model"], y=df["Bankrupt"], marker_color="#ef4444",
        text=df["Bankrupt"].round(1), texttemplate="%{text}%", textposition="outside"
    ))
    fig.add_trace(go.Bar(
        name="Safe", x=df["Model"], y=df["Safe"], marker_color="#10b981",
        text=df["Safe"].round(1), texttemplate="%{text}%", textposition="outside"
    ))
    fig.update_layout(
        title="Model Predictions Comparison", template="plotly_dark", height=400, barmode="group",
...
def chart_rf_importance(importances):
    ...
    df = pd.DataFrame({"Feature": [f.replace("_", " ").title() for f in FEATURES],
                       "Importance": importances}).sort_values("Importance").tail(12)
    colors = ['#6366f1' if i % 2 == 0 else '#8b5cf6' for i in range(len(df))]
    fig = go.Figure([go.Bar(
        x=df["Importance"], y=df["Feature"], orientation="h", marker_color=colors,
        text=df["Importance"].round(3), texttemplate="%{text}", textposition="outside"
    )])
    fig.update_layout(
        title="Random Forest Feature Importance (Top 12)", template="plotly_dark", height=500,
        margin=dict(l=20, r=20, t=60, b=20), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
...
def chart_z_gauge(z_score: float):
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            'steps': [
                {'range': [0, 1.81], 'color': 'rgba(239,68,68,.3)'},
...
def chart_sector_risk(stats: dict):
    ...
    fig.update_layout(
        title=f"Risk Distribution ({stats['n']} Companies)", template="plotly_dark", height=400,
        margin=dict(l=20, r=20, t=60, b=20), paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#cbd5e1"),
        showlegend=True
    )
    return fig
...
:root{
  --primary-bg:#0a0e27; --secondary-bg:#141b3d; --card-bg:rgba(20,27,61,.85);
  --card-border:rgba(99,102,241,.2); --text:#f8fafc; --muted:#94a3b8;
}
*{font-family:'Inter',sans-serif}
html,body,[data-testid="stAppViewContainer"]{
  background:linear-gradient(135deg,var(--primary-bg),var(--secondary-bg));background-attachment:fixed
}
.card{background:var(--card-bg);border:1px solid var(--card-border);border-radius:14px;padding:14px}
.metric-card{background:linear-gradient(135deg,rgba(99,102,241,.12),rgba(15,23,42,.85));border-radius:12px;padding:10px;text-align:center;border:1px solid rgba(99,102,241,.18)}
...
with st.sidebar:
    ...
    # Build models (cached) to show metrics
    try:
        pack_sidebar = build_models()
        st.metric("Active Models", len(pack_sidebar["models"]))
    ...
    st.markdown("---")
    st.markdown(
        "<div class='small'>Tip: use exchange suffixes for non-US tickers (e.g., RELIANCE.NS, SHOP.TO, VOD.L)</div>",
        unsafe_allow_html=True
    )
        ...
        with st.spinner(f"🔄 Analyzing {ticker}..."):
            try:
                ...
                with c1:
                    ...
                    with m1:
                        st.markdown(
                            f"""<div class="metric-card"><div class="metric-label">Market Cap</div><div class="metric-value">{fmt_curr(base.get('market_cap',0))}</div></div>""",
                            unsafe_allow_html=True
                        )
                    with m2:
                        st.markdown(
                            f"""<div class="metric-card"><div class="metric-label">Stock Price</div><div class="metric-value">{fmt_curr(base.get('stock_price',0))}</div></div>""",
                            unsafe_allow_html=True
                        )
                    with m3:
                        ind = base['industry'] if base['industry'] else "—"
                        st.markdown(
                            f"""<div class="metric-card"><div class="metric-label">Industry</div><div class="metric-value" style="font-size:1.0rem">{ind[:20]}</div></div>""",
                            unsafe_allow_html=True
                        )
                    st.markdown("</div>", unsafe_allow_html=True)
                ...
                with c2:
                    ...
                    st.markdown(
                        f"""<div class="progress-container" style="margin-top:12px;"><div class="progress-bar {bar}" style="width:{int(comb['combined_probability'])}%"></div></div>""",
                        unsafe_allow_html=True
                    )
                ...
                with m1:
                    ...
                    st.markdown(
                        f"<div class='small'><strong>Status:</strong> {z_status}<br><strong>Risk:</strong> {z_risk}<br><strong>Probability:</strong> {z_prob}%</div>",
                        unsafe_allow_html=True
                    )
                    st.markdown("</div>", unsafe_allow_html=True)
        ...
        with st.spinner(f"🔄 Analyzing {len(tks)} companies..."):
            ...
            for i, tk in enumerate(tks):
                try:
                    ...
                    rows.append({
                        "Ticker": tk, "Company": base["company_name"], "Sector": base["sector"],
                    ...
                    rows.append({
                        "Ticker": tk, "Company": "Error", "Sector": "-",
            ...
            if rows:
                ...
                if not valid.empty:
                    ...
                    fig = go.Figure([go.Bar(
                        x=valid["Ticker"], y=valid["Combined %"], marker_color=colors,
                        text=valid["Combined %"].round(1), texttemplate="%{text}%", textposition="outside"
                    )])
                    fig.update_layout(
                        title="Combined Bankruptcy Risk Comparison", template="plotly_dark", height=450,
                        margin=dict(l=20, r=20, t=60, b=20), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
...
with tabs[3]:
    ...
    fig.update_layout(
        title="Model Performance Comparison", template="plotly_dark", height=450, barmode="group",
...
with tabs[4]:
    ...
    if hist:
        tidy = [{
            "Timestamp": h["ts"], "Ticker": h["ticker"], "Company": h["company_name"], "Sector": h["sector"],
        ...
        if len(hist) > 1:
            ...
            fig.update_layout(
                title="Analysis Timeline - Risk Progression", template="plotly_dark", height=400,
                margin=dict(l=20, r=20, t=60, b=20), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
...
with tabs[5]:
    ...
    if st.session_state.history:
        last = st.session_state.history[-1]
        st.markdown(
            f"<div class='small'><strong>Latest Analysis:</strong><br><strong>Company:</strong> {last['company_name']}<br><strong>Ticker:</strong> {last['ticker']}<br><strong>Status:</strong> {last['combined']['status']}<br><strong>Risk:</strong> {last['combined']['combined_probability']}%</div>",
            unsafe_allow_html=True
        )
        ...
        with col2:
            if len(st.session_state.history) > 1:
                all_hist = [{
                    ...
                    "Status": h["combined"]["status"], "Risk_Level": h["combined"]["risk_level"]
