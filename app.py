# app.py — Enhanced Bankruptcy Prediction Dashboard
...
# CONFIG
# -------------------------------
st.set_page_config(page_title="Bankruptcy Prediction Dashboard", layout="wide", initial_sidebar_state="expanded")
st.set_page_config(
...
@@ -72,1084 +84,1738 @@ html, body, [data-testid="stAppViewContainer"] {
  backdrop-filter: blur(12px);
}

.metric-card {
  background: linear-gradient(135deg, rgba(99,102,241,0.15), rgba(15,23,42,0.85));
  border-radius: 14px;
  padding: 14px;
  text-align: center;
...
def _csv_from_result(tk, base, ratios, z, zprob, ml, comb):
    ...
    rfmt = {
        k: (round(v, 4) if abs(v) < 100 else round(v, 2)) for k, v in ratios.items()
    }
    for k, v in rfmt.items():
        out.write(f"{k},{v}\n")
...
def _first_nonempty_df(obj, attr_names):
    """Return the first non-empty DataFrame among given attribute names without truth-value checks."""
...
def fetch_yahoo_financials(
    ticker: str, prefer: str = "auto", retries: int = 2, pause: float = 0.8
):
    ...
    for _ in range(retries + 1):
        try:
            # info dict can be None for some tickers — guard it
            try:
                info = tkr.info or {}
            except Exception:
                info = {}

            # Collect annual / quarterly frames safely
            ...
            inc_quarter = _first_nonempty_df(
                tkr, ["quarterly_income_stmt", "quarterly_financials"]
            )
            bs_quarter = _first_nonempty_df(tkr, ["quarterly_balance_sheet"])
            cf_quarter = _first_nonempty_df(tkr, ["quarterly_cashflow"])
            ...
            if prefer == "annual":
                ...
                # auto: just take first non-empty among annual then quarterly
                def pick_first(a, q):
                    return a if isinstance(a, pd.DataFrame) and not a.empty else q
            ...
            if (inc_df is None or inc_df.empty) and (bs_df is None or bs_df.empty):
                ...
                raise ValueError(
                    "No financial statements available from Yahoo for this ticker."
                )
            ...
            cf  = _first_col(cf_df)
            cf = _first_col(cf_df)
            return {"info": info, "bs": bs, "inc": inc, "cf": cf}
...
def collect_financial_data(ticker: str, prefer="auto") -> dict:
    ...
    d = {
        ...
        "cash": _get_with_candidates(
            bs, ["Cash And Cash Equivalents", "CashAndCashEquivalents", "Cash"]
        ),
        "accounts_receivable": _get_with_candidates(
            bs, ["Accounts Receivable", "AccountsReceivable", "Receivables"]
        ),
        "inventory": _get_with_candidates(bs, ["Inventory"]),
        "total_liabilities": _get_with_candidates(bs, ["Total Liabilities Net Minority Interest",
                                                       "Total Liabilities",
        ...
        "current_liabilities": _get_with_candidates(
            bs, ["Current Liabilities", "CurrentLiabilities"]
        ),
        ...
        "stockholders_equity": _get_with_candidates(bs, ["Stockholders Equity",
                                                         "Total Equity Gross Minority Interest",
                                                         "StockholdersEquity",
                                                         "TotalEquityGrossMinorityInterest"]),

        "retained_earnings": _get_with_candidates(
        ...
        "operating_income": _get_with_candidates(
            inc, ["Operating Income", "OperatingIncome"]
        ),
        "ebit": _get_with_candidates(
            inc, ["EBIT", "Operating Income", "OperatingIncome"]
        ),
        ...
        "operating_cashflow": _get_with_candidates(cf, ["Operating Cash Flow", "OperatingCashFlow",
                                                        "Total Cash From Operating Activities"]),
        "capex": abs(_get_with_candidates(cf, ["Capital Expenditure", "CapitalExpenditures",
                                               "Capital Expenditures"])),
        "interest_expense": abs(
            _get_with_candidates(inc, ["Interest Expense", "InterestExpense"])
        )
        or 1.0,
        "operating_cashflow": _get_with_candidates(
            cf,
            [
                "Operating Cash Flow",
                "OperatingCashFlow",
...
def compute_ratios(d: dict) -> dict:
    TA = _safe_den(d["total_assets"])
    CL = _safe_den(d["current_liabilities"])
    TE = _safe_den(d["stockholders_equity"])
    TL = _safe_den(d["total_liabilities"])
    TR = _safe_den(d["total_revenue"])
    WC = d["current_assets"] - d["current_liabilities"]

    return {
        "working_capital_to_assets": WC / TA,
        "retained_earnings_to_assets": d["retained_earnings"] / TA,
        "ebit_to_assets": d["ebit"] / TA,
        "market_cap_to_liabilities": _safe_den(d["market_cap"]) / TL,
        "sales_to_assets": d["total_revenue"] / TA,

        "current_ratio": d["current_assets"] / CL,
        "quick_ratio": (d["current_assets"] - d["inventory"]) / CL,
        "cash_ratio": d["cash"] / CL,

        "debt_to_equity": TL / TE,
        "debt_to_assets": TL / TA,
        "long_term_debt_to_equity": d["long_term_debt"] / TE,
        "interest_coverage": d["ebit"] / _safe_den(d["interest_expense"]),

        "return_on_assets": (d["net_income"] / TA) * 100,
        "return_on_equity": (d["net_income"] / TE) * 100,
        "profit_margin": (d["net_income"] / TR) * 100,
        "gross_margin": (d["gross_profit"] / TR) * 100,
        "operating_margin": (d["operating_income"] / TR) * 100,
        "ebitda_margin": (d["ebitda"] / TR) * 100,

        "asset_turnover": d["total_revenue"] / TA,
        "receivables_turnover": d["total_revenue"] / _safe_den(d["accounts_receivable"]),
        "receivables_turnover": d["total_revenue"]
        / _safe_den(d["accounts_receivable"]),
        "inventory_turnover": d["total_revenue"] / _safe_den(d["inventory"]),

        "operating_cashflow_to_sales": d["operating_cashflow"] / TR,
        "free_cashflow_to_equity": d["free_cashflow"] / TE,
        "capex_to_revenue": d["capex"] / TR,
    }
...
def build_models():
    ...
    bankX = np.random.normal(
        ...
        scale=[0.1, 0.1, 0.05, 0.3, 0.3, 0.3, 1.0, 5, 10, 0.3, 2.0, 0.3, 5, 0.2, 0.2],
        size=(nb, 15),
    )
    bankY = np.ones(nb)
    ns = n - nb
    safeX = np.random.normal(
        loc=[0.4, 0.3, 0.15, 2.5, 1.5, 2.0, 1.0, 8, 12, 1.2, 5.0, 1.5, 10, 0.15, 0.4],
        scale=[0.15, 0.15, 0.08, 1.0, 0.5, 0.5, 0.5, 4, 8, 0.4, 3.0, 0.5, 5, 0.1, 0.15],
        size=(ns, 15),
    )
    safeY = np.zeros(ns)
...
def features_from_ratios(r):
    return np.array([r[k] for k in FEATURES], dtype=float).reshape(1, -1)

def features_from_ratios(r): return np.array([r[k] for k in FEATURES], dtype=float).reshape(1,-1)
...
def predict_all(r):
    ...
    return {"ensemble_prediction":"High Risk" if maj==1 else "Low Risk",
            ...
            "rf_features": pack["models"]["random_forest"].feature_importances_.tolist() if "random_forest" in pack["models"] else None}
    pack = build_models()
    ...
    return {
        ...
        "rf_features": (
            pack["models"]["random_forest"].feature_importances_.tolist()
            if "random_forest" in pack["models"]
            else None
        ),
    }
...
def chart_price(df: pd.DataFrame, ticker: str):
    """Plot price (candles or close line) + volume as a 2-row subplot."""
    if df is None or df.empty:
        return None

    have_ohlc = all(col in df.columns for col in ['Open', 'High', 'Low', 'Close'])
    have_ohlc = all(col in df.columns for col in ["Open", "High", "Low", "Close"])
    ...
    if have_ohlc:
        ...
        y_series = 'Adj Close' if 'Adj Close' in df.columns else 'Close'
        y_series = "Adj Close" if "Adj Close" in df.columns else "Close"
        fig.add_trace(
            go.Scatter(
                x=df["Date"],
    ...
    # Volume bars (vectorized colors to avoid ambiguous truth values)
    if "Volume" in df.columns:
        if have_ohlc and 'Open' in df.columns and 'Close' in df.columns:
...
def chart_z_components(components: dict):
    ...
    fig = go.Figure(data=[go.Bar(x=df["Component"], y=df["Value"], marker_color=colors,
    ...
    fig.update_layout(
        title="Altman Z-Score Components",
        template="plotly_dark",
        height=400,
        margin=dict(l=20, r=20, t=60, b=20),
        paper_bgcolor="rgba(0,0,0,0)",
...
def chart_ratio_radar(r: dict):
    keys = ["current_ratio", "quick_ratio", "cash_ratio", "debt_to_equity",
            "interest_coverage", "profit_margin", "operating_margin", "asset_turnover"]
    labels = ["Current Ratio", "Quick Ratio", "Cash Ratio", "Debt/Equity",
              "Interest Coverage", "Profit Margin", "Operating Margin", "Asset Turnover"]
    keys = [
        "current_ratio",
        "quick_ratio",
        "cash_ratio",
    ...
    fig.update_layout(
        template="plotly_dark",
        title="Key Financial Ratios Overview",
        height=450,
        polar=dict(
            radialaxis=dict(
                visible=True,
...
def chart_model_probs(ml: dict):
    ...
    for k, v in ml["individual_models"].items():
        rows.append({"Model": k.replace("_", " ").title(),
    ...
    fig.add_trace(go.Bar(name='Safe', x=df['Model'], y=df['Safe'], marker_color='#10b981',
                         ...
                         hovertemplate='<b>%{x}</b><br>Safe: %{y:.2f}%<extra></extra>'))
    ...
    fig.update_yaxes(
        showgrid=True, gridwidth=1, gridcolor="rgba(99, 102, 241, 0.1)", range=[0, 100]
    )
    return fig
...
def chart_rf_importance(importances):
    ...
    df = pd.DataFrame({"Feature": [f.replace("_", " ").title() for f in FEATURES],
                       "Importance": importances}).sort_values("Importance", ascending=True).tail(12)
    colors = ['#6366f1' if i % 2 == 0 else '#8b5cf6' for i in range(len(df))]
    fig = go.Figure(data=[go.Bar(x=df["Importance"], y=df["Feature"], orientation='h', marker_color=colors,
...
def chart_sector_risk(stats: dict):
    ...
    df = pd.DataFrame({"Status": ["Safe", "Gray Zone", "Distress"],
                       "Count": [dist["safe"], dist["gray"], dist["distress"]],
                       "Color": ["#10b981", "#f59e0b", "#ef4444"]})
    fig = go.Figure(data=[go.Bar(x=df["Status"], y=df["Count"], marker_color=df["Color"],
                                 text=df["Count"], textposition='outside',
                                 hovertemplate='<b>%{x}</b><br>Count: %{y}<extra></extra>')])
    fig.update_layout(title="Sector Risk Distribution", template="plotly_dark", height=350,
                      margin=dict(l=20, r=20, t=60, b=20), paper_bgcolor='rgba(0,0,0,0)',
                      plot_bgcolor='rgba(0,0,0,0)', font=dict(color='#cbd5e1'), showlegend=False)
    df = pd.DataFrame(
        {
            "Status": ["Safe", "Gray Zone", "Distress"],
            "Count": [dist["safe"], dist["gray"], dist["distress"]],
            "Color": ["#10b981", "#f59e0b", "#ef4444"],
        }
    )
...
def chart_z_gauge(z_score: float):
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            'bar': {'color': "#6366f1"},
            'bgcolor': "rgba(20, 27, 61, 0.5)",
            'borderwidth': 2,
            'bordercolor': "rgba(99, 102, 241, 0.3)",
            'steps': [
                {'range': [0, 1.81], 'color': 'rgba(239, 68, 68, 0.3)'},
                {'range': [1.81, 2.99], 'color': 'rgba(245, 158, 11, 0.3)'},
                {'range': [2.99, 5], 'color': 'rgba(16, 185, 129, 0.3)'}
            ],
...
<div class="dashboard-header">
  <div class="dashboard-title">🏛️ Bankruptcy Prediction Dashboard</div>
  <div class="dashboard-subtitle">Advanced Financial Analysis Using ML & Altman Z-Score</div>
  <div class="dashboard-meta">MGF 637 Final Project | Fall 2025</div>
</div>
...
with st.sidebar:
    ...
    prefer = st.selectbox("📊 Financials Preference", ["auto", "annual", "quarterly"], index=0,
                          help="Choose which financial statements to prioritize")
    price_years = st.select_slider("📈 Price History", options=[1, 2, 3, 5, 10], value=5,
                                   help="Select time range for stock price history")
    st.markdown('<hr>', unsafe_allow_html=True)
    st.markdown("<hr>", unsafe_allow_html=True)
    prefer = st.selectbox(
        "📊 Financials Preference",
        ["auto", "annual", "quarterly"],
        index=0,
        help="Choose which financial statements to prioritize",
    )
    price_years = st.select_slider(
        "📈 Price History",
        options=[1, 2, 3, 5, 10],
        value=5,
        help="Select time range for stock price history",
    ...
    st.markdown(
        """
    <div class="small">
    <strong>Global Tickers:</strong><br>
    • US: AAPL, MSFT<br>
    • India: RELIANCE.NS<br>
    • Canada: SHOP.TO<br>
    • UK: VOD.L
    </div>
    """, unsafe_allow_html=True)
...
tabs = st.tabs(
    ["🔍 Analyze", "⚖️ Compare", "🏢 Industry", "🤖 Models", "📜 History", "📥 Export"]
)
...
with tabs[0]:
    ...
    with col_inp:
        user_raw = st.text_input(label="Stock Ticker", value="",
    ...
    st.markdown(
        '<div class="small" style="margin-top:12px;">Quick Select:</div>',
        unsafe_allow_html=True,
    )
    q1, q2, q3, q4, q5, q6, q7, q8 = st.columns(8)
    quick_map = {"AAPL": q1, "MSFT": q2, "TSLA": q3, "AMZN": q4, "META": q5, "GOOGL": q6, "JPM": q7, "BAC": q8}
    quick_map = {
        "AAPL": q1,
        "MSFT": q2,
        "TSLA": q3,
    ...
    ticker = (
        selected_quick
        if selected_quick
        else (normalize_tickers(user_raw)[0] if user_raw else None)
    )
        ...
        with st.spinner(f"🔄 Analyzing {ticker}..."):
            try:
                ...
                result = {
                    ...
                    "ts": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    "ratios": {
                        k: (round(v, 4) if abs(v) < 100 else round(v, 2))
                ...
                st.markdown('<hr>', unsafe_allow_html=True)
                st.success(
                    f"✅ Successfully analyzed {base['company_name']} ({ticker})"
                )
                st.markdown("<hr>", unsafe_allow_html=True)

                # Top Row: Company Profile + Combined Assessment
                c1, c2 = st.columns([1.4, 1])
                with c1:
                    card_class = "success-card" if comb["risk_level"] == "Low" else ("warning-card" if comb["risk_level"] == "Medium" else "danger-card")
                    st.markdown(f'<div class="card {card_class}">', unsafe_allow_html=True)
                    card_class = (
                        "success-card"
                        if comb["risk_level"] == "Low"
                        else (
                            "warning-card"
                            if comb["risk_level"] == "Medium"
                            else "danger-card"
                        )
                    )
                    st.markdown(
                        f'<div class="card {card_class}">', unsafe_allow_html=True
                    )
                    ...
                    with left:
                        ...
                        st.markdown(
                            f"**Ticker:** {ticker} | **Sector:** {base['sector']}"
                        )
                    with right:
                        csv_bytes = _csv_from_result(
                            ticker, {"company_name": base["company_name"], "sector": base["sector"]},
                        ...
                        st.download_button("📥 Export", data=csv_bytes,
                                           ...
                                           use_container_width=True, key=f"dl_{ticker}")
                    ...
                    with m3:
                        show_industry = base['industry'] if base['industry'] else "—"
                        st.markdown(f"""<div class="metric-card">
                        show_industry = base["industry"] if base["industry"] else "—"
                        st.markdown(
                            f"""<div class="metric-card">
                            <div class="metric-label">Industry</div>
                            <div class="metric-value" style="font-size:1.2rem;">{show_industry[:20]}</div></div>""",
                                    unsafe_allow_html=True)
                            unsafe_allow_html=True,
                        )
                    st.markdown("</div>", unsafe_allow_html=True)
                ...
                with c2:
                    ...
                    badge_class = "safe" if comb["status"] == "Safe" else ("gray" if comb["status"] == "Gray Zone" else "distress")
                    st.markdown(f'<div class="badge {badge_class}" style="margin:12px 0;">{comb["status"]}</div>',
                                unsafe_allow_html=True)
                    badge_class = (
                        "safe"
                        if comb["status"] == "Safe"
                        else ("gray" if comb["status"] == "Gray Zone" else "distress")
                    )
                    st.markdown(
                        f'<div class="badge {badge_class}" style="margin:12px 0;">{comb["status"]}</div>',
                        unsafe_allow_html=True,
                    )
                    b1, b2 = st.columns(2)
                    with b1:
                        ...
                        st.metric(
                            "Agreement", "✓ Yes" if comb["model_agreement"] else "✗ No"
                        )
                    with b2:
                        st.metric("Combined Risk", f"{comb['combined_probability']}%")
                        st.metric("Confidence", f"{ml['confidence']}%")
                    bar_class = "safe" if comb["combined_probability"] < 20 else ("warning" if comb["combined_probability"] < 50 else "danger")
                    st.markdown(f"""
                    bar_class = (
                        "safe"
                        if comb["combined_probability"] < 20
                        else (
                            "warning" if comb["combined_probability"] < 50 else "danger"
                        )
                    )
                    st.markdown(
                        f"""
                    <div class="progress-container" style="margin-top:16px;">
                        <div class="progress-bar {bar_class}" style="width: {int(comb['combined_probability'])}%"></div>
                    </div>
                    <div class="small" style="margin-top:8px; text-align:center;">{comb['recommendation']}</div>
                    """, unsafe_allow_html=True)
                    """,
                        unsafe_allow_html=True,
                    )
                ...
                # Middle Row: Z-Score Gauge + Price Chart
                m1, m2 = st.columns([1, 2])
                with m1:
                    st.markdown('<div class="card">', unsafe_allow_html=True)
                    fig_gauge = chart_z_gauge(z)
                    ...
                    st.markdown(
                        f"""
                    <div class="small">
                        <strong>Status:</strong> {z_status}<br>
                        <strong>Risk:</strong> {z_risk}<br>
                        <strong>Probability:</strong> {z_prob}%
                    </div>
                    """, unsafe_allow_html=True)
                    """,
                        unsafe_allow_html=True,
                ...
                with m2:
                    st.markdown('<div class="card">', unsafe_allow_html=True)
                    price_df = fetch_price_history(ticker, period=f"{price_years}y")
                ...
                # Bottom Row: Model Predictions + Feature Importance
                ...
                tab_cols = st.columns(len(ratio_categories))
                for idx, (category, ratio_keys) in enumerate(ratio_categories.items()):
                    with tab_cols[idx]:
                        st.markdown(f"**{category}**")
                        cat_data = []
                        for key in ratio_keys:
                            if key in result["ratios"]:
                                cat_data.append({"Metric": key.replace("_", " ").title(), "Value": result["ratios"][key]})
                                cat_data.append(
                                    {
                                        "Metric": key.replace("_", " ").title(),
                                        "Value": result["ratios"][key],
                                    }
                                )
                        if cat_data:
                            st.dataframe(pd.DataFrame(cat_data), use_container_width=True, hide_index=True, height=250)
                            st.dataframe(
                ...
                st.markdown('<div class="card danger-card">', unsafe_allow_html=True)
                ...
                • Verify the ticker symbol is correct<br>
                • Add exchange suffix for international stocks (e.g., .NS, .TO, .L)<br>
                • Some tickers may have limited financial data on Yahoo Finance<br>
                • Try switching financials preference between annual and quarterly
                </div>
                """, unsafe_allow_html=True)
                """,
                    unsafe_allow_html=True,
            ...
            for idx, tk in enumerate(tks):
                try:
                    ...
                    rows.append({"Ticker": tk, "Company": base["company_name"], "Sector": base["sector"],
                                 ...
                                 "Status": comb["status"], "Risk Level": comb["risk_level"]})
                    rows.append(
                        {
                            "Ticker": tk,
                    ...
                    rows.append({"Ticker": tk, "Company": "Error", "Sector": "-",
                                 "Z-Score": None, "ML Risk %": None, "Combined %": None,
                ...
                progress_bar.progress((idx + 1) / len(tks))
            progress_bar.empty()
            if rows:
                dfc = pd.DataFrame(rows)
                st.dataframe(dfc, use_container_width=True, hide_index=True)
                valid_data = dfc[dfc["Combined %"].notna()].copy()
                if not valid_data.empty:
                    fig = go.Figure()
                    colors = []
                    for status in valid_data["Status"]:
                        colors.append("#10b981" if status=="Safe" else "#f59e0b" if status=="Gray Zone" else "#ef4444")
                    fig.add_trace(go.Bar(x=valid_data["Ticker"], y=valid_data["Combined %"], marker_color=colors,
                                         text=valid_data["Combined %"].round(1), texttemplate='%{text}%',
                                         textposition='outside'))
                    fig.update_layout(title="Combined Bankruptcy Risk Comparison", template="plotly_dark", height=450,
                        ...
                        colors.append(
                            "#10b981"
                            if status == "Safe"
                            else "#f59e0b" if status == "Gray Zone" else "#ef4444"
                        )
                    fig.add_trace(
                        go.Bar(
                            x=valid_data["Ticker"],
                            y=valid_data["Combined %"],
                            marker_color=colors,
                            text=valid_data["Combined %"].round(1),
                            texttemplate="%{text}%",
                            textposition="outside",
                        )
                    )
                    fig.update_layout(
                        title="Combined Bankruptcy Risk Comparison",
                        template="plotly_dark",
                        height=450,
                        margin=dict(l=20, r=20, t=60, b=20),
                        paper_bgcolor="rgba(0,0,0,0)",
    ...
    if sectors:
        ...
        if sec:
            ...
            if stats:
                ...
                for label, val in [("Companies", stats['n']), ("Avg Z-Score", stats['z_mean']),
                                   ("Median Z", stats['z_med']), ("Min Z", stats['z_min']),
                                   ("Max Z", stats['z_max'])]:
                    with [m1, m2, m3, m4, m5][["Companies","Avg Z-Score","Median Z","Min Z","Max Z"].index(label)]:
                        st.markdown(f"""<div class="metric-card">
                for label, val in [
                    ("Companies", stats["n"]),
                    ("Avg Z-Score", stats["z_mean"]),
                    ("Median Z", stats["z_med"]),
                    ("Min Z", stats["z_min"]),
                    ("Max Z", stats["z_max"]),
                ]:
                    with [m1, m2, m3, m4, m5][
                ...
                st.plotly_chart(
                    chart_sector_risk(stats),
                    use_container_width=True,
                    config={"displayModeBar": False},
                )
                st.markdown("#### Sector Companies")
                sector_df = pd.DataFrame(stats["rows"])
                st.dataframe(sector_df, use_container_width=True, hide_index=True)
    else:
        st.info("📊 No sector data available yet. Run some analyses to populate industry statistics.")
        st.info(
            "📊 No sector data available yet. Run some analyses to populate industry statistics."
        )
    st.markdown("</div>", unsafe_allow_html=True)
...
with tabs[3]:
    ...
    st.dataframe(
        pd.DataFrame(rows), use_container_width=True, hide_index=True, height=250
    )
    fig = go.Figure()
    metrics_to_plot = ["Accuracy", "Precision", "Recall", "F1-Score", "ROC-AUC"]
    colors_map = {"Accuracy": "#6366f1", "Precision": "#8b5cf6", "Recall": "#ec4899", "F1-Score": "#10b981", "ROC-AUC": "#f59e0b"}
    colors_map = {
        "Accuracy": "#6366f1",
        "Precision": "#8b5cf6",
        "Recall": "#ec4899",
    ...
    for metric in metrics_to_plot:
        values = [float(v.strip("%")) for v in df_rows[metric]]
        fig.add_trace(go.Bar(name=metric, x=df_rows["Model"], y=values, marker_color=colors_map[metric],
    ...
    fig.update_layout(title="Model Performance Comparison", template="plotly_dark", height=450, barmode='group',
                      margin=dict(l=20, r=20, t=60, b=80), paper_bgcolor='rgba(0,0,0,0)',
                      plot_bgcolor='rgba(0,0,0,0)', font=dict(color='#cbd5e1'),
    ...
    Training Data: 1,000 synthetic samples (33% bankrupt, 67% safe)<br>
    Features: 15 financial ratios and metrics<br>
    Split: 80% train / 20% test<br>
    Ensemble: Majority voting with probability averaging
    </div>
    """, unsafe_allow_html=True)
    """,
        unsafe_allow_html=True,
...
with tabs[4]:
    ...
    if hist:
        tidy = []
        for h in hist:
            tidy.append({"Timestamp": h["ts"], "Ticker": h["ticker"], "Company": h["company_name"],
                         "Sector": h["sector"], "Z-Score": h["altman"]["score"],
        ...
        if len(hist) > 1:
            ...
            fig.update_layout(title="Analysis Timeline - Risk Progression", template="plotly_dark", height=400,
                              margin=dict(l=20, r=20, t=60, b=20), paper_bgcolor='rgba(0,0,0,0)',
                              plot_bgcolor='rgba(0,0,0,0)', font=dict(color='#cbd5e1'),
                              xaxis_title="Analysis Number", yaxis_title="Bankruptcy Risk %")
            fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor='rgba(99, 102, 241, 0.1)')
            fig.update_yaxes(showgrid=True, gridwidth=1, gridcolor='rgba(99, 102, 241, 0.1)', range=[0, 100])
            st.plotly_chart(fig, use_container_width=True, config={"displayModeBar": False})
            fig.add_trace(
            ...
            fig.update_layout(
                title="Analysis Timeline - Risk Progression",
                template="plotly_dark",
                height=400,
                margin=dict(l=20, r=20, t=60, b=20),
                paper_bgcolor="rgba(0,0,0,0)",
        ...
        st.info(
            "📊 No analysis history yet. Start by analyzing a company in the Analyze tab."
        )
    st.markdown("</div>", unsafe_allow_html=True)
...
with tabs[5]:
    ...
    if st.session_state.history:
        ...
        st.markdown(
            f"""
        <div class="small">
        <strong>Latest Analysis:</strong><br>
        <strong>Company:</strong> {last['company_name']}<br>
        <strong>Ticker:</strong> {last['ticker']}<br>
        <strong>Status:</strong> {last['combined']['status']}<br>
        <strong>Risk:</strong> {last['combined']['combined_probability']}%
        </div>
        """, unsafe_allow_html=True)
        """,
            unsafe_allow_html=True,
        ...
        with col2:
            if len(st.session_state.history) > 1:
                ...
                for h in st.session_state.history:
                    all_hist.append({"Timestamp": h["ts"], "Ticker": h["ticker"], "Company": h["company_name"],
                                     "Sector": h["sector"], "Z-Score": h["altman"]["score"],
                                     "Z-Status": h["altman"]["status"],
...
st.markdown(
    """
<div style="text-align:center;margin-top:24px;padding:16px;border-top:1px solid rgba(99,102,241,0.2);color:#94a3b8">
  <p><strong>Powered by:</strong> Altman Z-Score | ML (Logistic Regression, Random Forest, Gradient Boosting, XGBoost)</p>
  ...
  <p>MGF 637 Financial Analytics | Fall 2025</p>
</div>
""", unsafe_allow_html=True)
""",
    unsafe_allow_html=True,
